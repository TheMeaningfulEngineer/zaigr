...
from ..conftest import REPO_ROOT, ProjectTestEnv, err_msg, spawn_interactive
...
def _project_images(project):
    return sorted(
        ...
        if path.suffix == ".qcow2" and not path.name.endswith(".wip")
    )
...
def test_setup_firewall_is_shown_and_allows_host_access(project, setup_factory):
    ...
    assert "# project setups" in stdout
    assert "deb.debian.org" in stdout

    run(f"{zaigr} project vm start", timeout=120)
    run(
...
def test_zaigr_inside_firewall_remove_blocks_removed_domain(project):
    ...
    opensnitch_pid_before = run(
        f"{zaigr} project vm exec -- pgrep -x opensnitchd",
        timeout=20,
    ).strip()
    ...
    opensnitch_pid_after_remove = run(
        f"{zaigr} project vm exec -- pgrep -x opensnitchd",
        timeout=20,
    ).strip()
    assert opensnitch_pid_after_remove == opensnitch_pid_before
    ...
    # Rule reload is asynchronous for restoration as well as removal.
    # Retry only the blocked-connection result, not unrelated curl errors.
    for _ in range(12):
        restored, stderr, rc = _run(
            f"{zaigr} project vm exec -- curl -4 -fsS -o /dev/null "
            f"--connect-timeout 3 --max-time 8 {test_url}",
            cwd=project.cwd,
            env=project.env,
        ...
        if rc != int(blocked_curl_status):
            break
        time.sleep(1)
    assert rc == 0, err_msg(restored, stderr)
    opensnitch_pid_after_restore = run(
        f"{zaigr} project vm exec -- pgrep -x opensnitchd",
        timeout=20,
    ).strip()
    assert opensnitch_pid_after_restore == opensnitch_pid_before


@pytest.mark.timeout(300)
def test_running_setup_firewall_applies_without_restarting_opensnitch(project, setup_factory):
    """A running setup activates its domain list without restarting OpenSnitch."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    setup_factory(
        setup_name="live-firewall-update",
        script_content="true",
        firewall=["pypi.org"],
    )
    ...
    run(
        f"{zaigr} project setup run live-firewall-update",
        timeout=180,
    )
    run(
        f"{zaigr} project vm exec -- curl -4 -fsS -o /dev/null "
        "--connect-timeout 3 --max-time 8 https://pypi.org",
        timeout=20,
    )

    opensnitch_pid_after = run(
        f"{zaigr} project vm exec -- pgrep -x opensnitchd",
        timeout=20,
    ).strip()
    assert opensnitch_pid_after == opensnitch_pid_before
...
def test_global_projects_list_shows_compact_and_full_paths(project, tmp_path):
    """`list` exposes the agreed project table."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    ...
    # Simulate a saved project created by an older binary while its VM is live.
    base_version_path.write_text(f"{previous_base}\n", encoding="utf-8")

    running = run(
        f"{zaigr} global projects list",
        timeout=10,
    )
    running_row = next(
        line for line in running.splitlines() if line.startswith(project.cwd.name)
    )
    assert running_row.split()[1] == "running,"
    assert running_row.split()[2] == "outdated"
    ...
    running_with_base = run(
        f"{zaigr} global projects list --show-base-image",
        timeout=10,
    )
    running_base_row = next(
        line for line in running_with_base.splitlines() if line.startswith(project.cwd.name)
    )
    assert running_base_row.split()[1:3] == ["running,", "outdated"]
    assert running_base_row.split()[-3:-1] == [previous_base, required_base]
    assert base_version_path.read_text(encoding="utf-8") == f"{previous_base}\n"

    base_version_path.write_text(f"{required_base}\n", encoding="utf-8")

    run(f"{zaigr} project vm stop")
    store_dir = project.store_dir

    stdout = run(
        f"{project.zaigr_bin} global projects list",
        timeout=10,
    )
    ...
    assert "SHELLS" not in stdout
    assert "CAPTURE" not in stdout
    assert store_dir.name in stdout
    assert "off" in stdout
    assert "1" in stdout
    assert "512.0MiB" in stdout
    assert project.cwd.name in stdout

    full = run(
        f"{project.zaigr_bin} global projects list --full-paths",
        timeout=10,
    )
    assert str(project.cwd) in full


def test_global_projects_list_explains_stale_runtime_state(project):
    """`global projects list` tells users how to clear stale VM runtime state."""
    run = partial(_run, cwd=project.cwd, env=project.env)
    store_dir = project.store_dir
    ...
    (store_dir / "base-image-version").write_text("test-base\n", encoding="utf-8")
    (store_dir / "image-format").write_text("direct-base-v1\n", encoding="utf-8")
    base_image = project.home / ".zaigr" / "base-images" / "test-base" / "base.qcow2"
    base_image.parent.mkdir(parents=True)
    stdout, stderr, rc = run(
        ["qemu-img", "create", "-f", "qcow2", base_image, "100G"],
        timeout=30,
    )
    assert rc == 0, err_msg(stdout, stderr)
    base_image.chmod(0o444)
    stdout, stderr, rc = run(
        [
            "qemu-img",
            "create",
    ...
    assert f"project {project.cwd.name} ({store_dir.name}) has leftover VM runtime state" in stderr
...
def test_global_projects_inspection_and_kill_work_with_legacy_store(
    test_root, zaigr_bin, zaigr_home, tmp_path
):
    """Legacy metadata remains inspectable and does not prevent runtime cleanup."""
    # This metadata-only environment owns no VM requiring project teardown.
    project_dir = test_root / "legacy-project"
    project_dir.mkdir()
    project = ProjectTestEnv(cwd=project_dir, zaigr_bin=Path(zaigr_bin), home=zaigr_home)
    store_dir = project.store_dir
    ...
    (store_dir / "cpus").write_text("1\n", encoding="utf-8")
    (store_dir / "config").write_text("disk=100G\n", encoding="utf-8")
    (store_dir / "base-image-version").write_text("legacy-base\n", encoding="utf-8")
    awaiting_dir = store_dir / "awaiting-commit"
    awaiting_dir.mkdir()
    (awaiting_dir / "001-legacy.script").write_text(
        "printf 'legacy setup state\\n'\n",
        encoding="utf-8",
    )
    legacy_image = store_dir / "image-legacy.qcow2"
    stdout, stderr, rc = _run(
    ...
    runtime_dir = store_dir / "runtime"
    runtime_dir.mkdir()
    (runtime_dir / "ssh-port").write_text("40022\n", encoding="utf-8")
    sock_path = runtime_dir / "vm.sock"
    sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    try:
        sock.bind(str(sock_path))
    ...
    entries_before = sorted(str(path.relative_to(store_dir)) for path in store_dir.rglob("*"))
    files_before = {
        str(path.relative_to(store_dir)): path.read_bytes()
        for path in store_dir.rglob("*")
        if path.is_file()
    }
    socket_inode_before = sock_path.stat().st_ino
    ...
    assert store_dir.name in stdout
    assert "stale" in stdout
    assert "zaigr project rebuild" not in (stdout + stderr)
    assert "zaigr project clean" not in (stdout + stderr)
    assert "zaigr project delete" not in (stdout + stderr)
    assert f"zaigr global projects kill {store_dir.name}" in stderr
    assert sorted(str(path.relative_to(store_dir)) for path in store_dir.rglob("*")) == entries_before
    assert {
        str(path.relative_to(store_dir)): path.read_bytes()
        for path in store_dir.rglob("*")
        if path.is_file()
    } == files_before
    assert sock_path.stat().st_ino == socket_inode_before

    stdout, stderr, rc = _run(
        f"{project.zaigr_bin} global projects details {store_dir.name}",
        cwd=tmp_path,
        env=project.env,
        timeout=10,
    )
    assert rc == 0, err_msg(stdout, stderr)
    assert "resources: cpu=1, ram=512.0MiB" in stdout
    assert "disk: " in stdout
    assert "incompatible legacy project store" in stderr
    ...
    assert {
        ...
        for path in store_dir.rglob("*")
        if path.is_file()
    } == {name: body for name, body in files_before.items() if not name.startswith("runtime/")}
...
def test_global_projects_details_prints_project_store_state(project, tmp_path, setup_factory):
    ...
    assert "status: off" in stdout
    assert "resources: cpu=1, ram=512.0MiB" in stdout
    assert "disk: " in stdout
    assert "applied (1):" in stdout
...
def test_directory_setup_merges_multiple_firewall_files(project):
    """Every direct `*.firewall` file contributes working network access."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    ...
    run(f"{project.zaigr_bin} project vm start", timeout=120)
    run(
        f"{project.zaigr_bin} project vm exec -- "
        "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 https://go.dev",
        timeout=20,
    )
    run(
        f"{project.zaigr_bin} project vm exec -- "
        "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 https://api.github.com",
        timeout=20,
    )
...
def test_shell_reuses_running_project_and_skips_redundant_firewall_sync(project, setup_factory):
    """A second shell attach on a running VM should not re-sync applied firewall state."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    setup_factory(
...
@pytest.mark.timeout(360)
def test_firewall_sync_cannot_break_exec_or_shell_ssh(project, setup_factory):
    """Boot-time firewall synchronization preserves exec and shell SSH."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    setup_factory(
        setup_name="ssh-firewall-invariant",
        script_content="true",
        firewall=["deb.debian.org"],
    )

    run(
        f"{zaigr} project setup run ssh-firewall-invariant",
        input="y\n",
        timeout=180,
    )

    synced_shell = spawn_interactive(
        f"{zaigr} shell",
        cwd=project.cwd,
        env=project.env,
        timeout=120,
    ...
    try:
        synced_shell.expect(USER_PROMPT, timeout=120)
        synced_shell.setecho(False)
        synced_shell.sendline(
            "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 "
            "http://deb.debian.org && printf 'FIREWALL_ACCESS_OK\\n'"
        )
        synced_shell.expect(r"FIREWALL_ACCESS_OK", timeout=20)
        synced_shell.expect(USER_PROMPT)
        synced_shell.sendline("exit")
        synced_shell.expect(r"\Z")
    finally:
        synced_shell.close(force=True)

    first_exec = run(
        f"{zaigr} project vm exec -- printf NEW_SSH_AFTER_SYNC",
        timeout=20,
    )
    assert first_exec == "NEW_SSH_AFTER_SYNC"


@pytest.mark.timeout(420)
@pytest.mark.parametrize("stop_args", [[], ["--abrupt"]], ids=["graceful", "abrupt"])
def test_applied_firewall_survives_repeated_vm_lifecycle(project, setup_factory, stop_args):
    """Repeated public stop/start cycles cannot strand a firewall-enabled VM."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    setup_factory(
        setup_name="ssh-lifecycle-invariant",
        script_content="true",
        firewall=["example.org", "pypi.org"],
    )

    run(
        f"{zaigr} project setup run ssh-lifecycle-invariant",
        input="y\n",
        timeout=180,
    )
    ...
    for cycle in range(3):
        probe = run(
            f"{zaigr} project vm exec -- printf SSH_CYCLE_{cycle}",
            timeout=20,
        )
        assert probe == f"SSH_CYCLE_{cycle}"
        # Keep setup files durable while exercising force-kill process cleanup.
        run(f"{zaigr} project vm exec -- sync", timeout=20)
        stopped = run([zaigr, "project", "vm", "stop", *stop_args], timeout=60)
    ...
    final_probe = run(
        f"{zaigr} project vm exec -- printf SSH_AFTER_FINAL_START",
        timeout=20,
    )
    assert final_probe == "SSH_AFTER_FINAL_START"
...
@pytest.mark.timeout(300)
def test_setup_apt_confirmation_is_reported_before_execution_and_rebuild(project):
    """APT commands that need input are identified before non-interactive replay."""
    zaigr = project.zaigr_bin
    ...
    setup_script = project.cwd / ".zaigr" / "setups" / "local" / "setup.script"
    setup_script.write_text(
        "#!/bin/bash\n"
        "set -eu\n"
        "# documentation example: cat <<COMMENTED_EXAMPLE\n"
        "printf '%s\\n' '<<QUOTED_EXAMPLE' > /tmp/quoted-example\n"
        "printf '%s\\n' \"$((1 << 2))\" > /tmp/arithmetic-example\n"
        "grep -q x <<<\"x\"\n"
        "if false; then\n"
        "  DEBIAN_FRONTEND=noninteractive apt-get install \\\n"
        "    curl\n"
        "fi\n"
        "cat > /tmp/apt-example <<'APT_EXAMPLE'\n"
        "apt-get install curl\n"
        "APT_EXAMPLE\n"
        "printf 'apt-preflight-ran\\n' > /etc/zaigr-apt-preflight\n",
        encoding="utf-8",
    )
    ...
    assert rc != 0, err_msg(stdout, stderr)
    combined = stdout + stderr
    assert "Setup scripts contain APT commands without automatic confirmation" in combined
    assert "[Project local] local" in combined
    assert ".zaigr/setups/local/setup.script:8" in combined
    assert "apt-get install curl" in combined
    assert combined.count("apt-get install curl") == 1
    assert "Setup scripts run non-interactively" in combined
    assert "Add `-y` or `--assume-yes`" in combined
    assert "setup execution aborted before running setup scripts" in combined
    assert "Booting image for modification" not in combined
    ...
    assert rc == 0, err_msg(stdout, stderr)
    assert "Continue with setup execution anyway? [y/N]" in (stdout + stderr)
    ...
    combined = stdout + stderr
    assert "Setup scripts contain APT commands without automatic confirmation" in combined
    assert "project rebuild aborted before setup execution" in combined
    assert "original project left unchanged" in combined
    assert "Booting image for modification" not in combined
    ...
    assert rc == 0, err_msg(stdout, stderr)
    assert "APT commands without automatic confirmation" not in (stdout + stderr)
    ...
    assert rc == 0, err_msg(stdout, stderr)
    stdout, stderr, rc = _run(
        f"{zaigr} project vm exec -- cat /etc/zaigr-apt-preflight",
        cwd=project.cwd,
        env=project.env,
        timeout=20,
    )
    assert rc == 0, err_msg(stdout, stderr)
    assert stdout.strip() == "apt-preflight-ran"


@pytest.mark.timeout(120)
def test_project_setup_run_force_reruns_applied_setup(project, setup_factory):
    """`project setup run --force` reruns a setup that is already applied."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    ...
    assert (
        ":: Skipping setups already applied at the current version: force-project-setup-run"
        in skipped
    )
...
@pytest.mark.timeout(360)
def test_project_setup_run_force_on_running_vm_replaces_applied_setup(
    project, setup_factory
):
    """A forced setup rerun applies immediately and replaces its applied record."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin
    ...
    assert "awaiting image commit" not in forced.lower()
    assert "commit this setup" not in forced.lower()

    status = run(f"{zaigr} project status", timeout=10)
    assert "status: running" in status

    old_effect = run(
        f"{zaigr} project vm exec -- cat /etc/zaigr-force-running-old",
        timeout=20,
    )
    new_effect = run(
        f"{zaigr} project vm exec -- cat /etc/zaigr-force-running-new",
        timeout=20,
    )
    assert old_effect.strip() == "old"
    assert new_effect.strip() == "new"
    ...
    assert "applied (1):" in list_output
    assert list_output.count("force-running-marker") == 1


@pytest.mark.timeout(120)
def test_project_setup_list_defaults_to_only_applied_setups(project, setup_factory):
    """Default list and status output contain no removed setup-state sections."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    zaigr = project.zaigr_bin

    setup_factory(
        setup_name="applied-list",
        script_content="printf 'applied-list-run\\n'",
    )
    run(
        f"{zaigr} project setup run applied-list",
        input="y\n",
        timeout=120,
    )
    ...
    assert "applied (1):" in list_output
    assert "applied-list" in list_output
    assert "awaiting" not in list_output.lower()
    assert "pending" not in list_output.lower()
    assert "failed setups" not in list_output.lower()

    status = run(f"{zaigr} project status", timeout=10)
    assert "applied (1):" in status
    assert "applied-list" in status
    assert "awaiting" not in status.lower()
    assert "pending" not in status.lower()
    assert "failed setups" not in status.lower()


def test_project_setup_removed_commands_and_state_flags_are_rejected(project):
    """Removed setup inspection commands and state flags are not accepted."""
    for args in (
        "project setup show",
        "project setup list --committed",
        "project setup list --awaiting-commit",
        "project setup list --failed",
    ):
        stdout, stderr, rc = _run(
            f"{project.zaigr_bin} {args}",
            cwd=project.cwd,
            env=project.env,
            timeout=10,
        )
        assert rc != 0, err_msg(stdout, stderr)
        assert "unknown" in (stdout + stderr).lower()
...
def test_project_setup_capture_records_endpoints_and_discard_clears_capture(
    ...
    try:
        ...
        nested, stderr, rc = _run(
            f"{project.zaigr_bin} project setup capture start",
            cwd=project.cwd,
            env=project.env,
            timeout=20,
        )
        assert rc != 0, err_msg(nested, stderr)
        assert "project setup capture start is running in another terminal" in nested + stderr
        ...
        try:
            ...
            try:
                if capture_again.isalive():
                    ...
                    capture_again.expect(pexpect.EOF, timeout=30)
            finally:
                capture_again.close(force=True)
    finally:
        try:
            ...
            # A failure before capture creation can leave nothing to discard.
            if rc != 0:
                assert (
                    "no active setup capture to discard" in stdout + stderr
                    or "requires a project" in stdout + stderr
                ), err_msg(stdout, stderr)
            else:
                assert "discarded setup capture" in stdout.lower()
...
@pytest.mark.timeout(240)
def test_project_setup_capture_start_rejects_a_second_capture(project):
    """Only capture review, accept, and discard may start during capture."""
    capture = spawn_interactive(
        f"{project.zaigr_bin} project setup capture start",
        cwd=project.cwd,
        env=project.env,
    ...
    try:
        ...
        assert rc != 0, err_msg(stdout, stderr)
        assert "project setup capture start is running in another terminal" in stdout + stderr
    finally:
        try:
            if capture.isalive():
                capture.sendline("exit")
            ...
            capture.close(force=True)
            stdout, stderr, rc = _run(
                f"{project.zaigr_bin} project setup capture discard",
                cwd=project.cwd,
                env=project.env,
                input="y\n",
                timeout=30,
...
def test_project_setup_capture_can_attach_to_running_vm(project):
    ...
    try:
        ...
        active_restart, stderr, rc = _run(
            f"{zaigr} project setup capture start",
            cwd=project.cwd,
            env=project.env,
            timeout=20,
        )
        assert rc != 0, err_msg(active_restart, stderr)
        assert "project setup capture start is running in another terminal" in active_restart + stderr
...
def test_global_firewall_allowlist_applies_to_project_starts(project, setup_factory, tmp_path):
    ...
    github, stderr, rc = run(
        f"{zaigr} project vm exec -- "
        "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 https://github.com",
        timeout=20,
    )
    assert rc == 0, err_msg(github, stderr)
    pypi, stderr, rc = run(
        f"{zaigr} project vm exec -- "
        "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 https://pypi.org",
        timeout=20,
    )
    assert rc == 0, err_msg(pypi, stderr)
    ...
    assert rc == 28, err_msg(github, stderr)
    pypi, stderr, rc = run(
        f"{zaigr} project vm exec -- "
        "curl -4 -fsS -o /dev/null --connect-timeout 3 --max-time 8 https://pypi.org",
        timeout=20,
...
def test_shell_root_dirty_prompt_rejects_then_accept_marks_project_image_dirty(project):
    ...
    run(f"{project.zaigr_bin} project vm stop", timeout=60)
    stdout, stderr, rc = _run(
        f"{project.zaigr_bin} project rebuild",
        cwd=project.cwd,
        env=project.env,
        timeout=30,
    )
    assert rc != 0, err_msg(stdout, stderr)
    combined = stdout + stderr
    assert "has changes made with zaigr shell --root" in combined
...
def test_two_projects_keep_setup_state_separate(project_factory, setup_factory):
    ...
    assert "applied (1):" in status_a
    assert "applied (0):" in status_b

    run = partial(_checked_run, cwd=project_a.cwd, env=project_a.env)
    list_a = run(
        f"{project_a.zaigr_bin} project setup list --applied",
        timeout=10,
    )
    assert "applied (1):" in list_a
    assert "hello" in list_a

    run = partial(_checked_run, cwd=project_b.cwd, env=project_b.env)
    list_b = run(
        f"{project_b.zaigr_bin} project setup list --applied",
        timeout=10,
    )
    assert "applied (0):" in list_b
    assert "hello" not in list_b


@pytest.mark.timeout(240)
def test_setup_run_applies_immediately_to_running_project(project, setup_factory):
    """A setup applies to the authoritative image while its VM keeps running."""
    run = partial(_checked_run, cwd=project.cwd, env=project.env)
    setup_factory(
        setup_name="running-direct-apply",
        script_content=(
            "printf 'RUNNING_DIRECT_APPLY_RAN\\n'\n"
            "printf 'running-direct-apply-present\\n' > /etc/zaigr-running-direct-apply"
        ),
    )
    ...
    setup = run(
        f"{project.zaigr_bin} project setup run running-direct-apply",
        timeout=60,
    )
    assert "RUNNING_DIRECT_APPLY_RAN" in setup
    assert "awaiting image commit" not in setup.lower()
    assert "commit" not in setup.lower()
    ...
    assert "status: running" in status
    assert "applied (1):" in status
    assert "running-direct-apply" in status
    assert "This VM had a failed setup once." not in status

    marker = run(
        f"{project.zaigr_bin} project vm exec -- cat /etc/zaigr-running-direct-apply",
        timeout=20,
    )
    assert marker.strip() == "running-direct-apply-present"
    ...
    list_output = run(
        f"{project.zaigr_bin} project setup list --applied",
        timeout=10,
    )
    assert "applied (1):" in list_output
    assert list_output.count("running-direct-apply") == 1


@pytest.mark.timeout(180)
def test_project_clean_resets_project_image_and_setup_status(project, setup_factory):
    """`project clean` replaces the overlay and clears all setup state."""
    run = partial(_run, cwd=project.cwd, env=project.env)
    setup_factory(
    ...
    marker, stderr, rc = run(
        f"{project.zaigr_bin} project vm exec -- cat /etc/zaigr-clean-mark",
        timeout=30,
    )
    ...
    assert rc == 0, err_msg(list_output, stderr)
    assert "applied (0):" in list_output
    ...
    marker_cleaned, stderr, rc = run(
        f"{project.zaigr_bin} project vm exec -- "
        "bash -lc \"test ! -e /etc/zaigr-clean-mark && printf 'marker-cleaned\\n'\"",
        timeout=30,
    )
...
""",
    ...
    assert "applied (1):" in status
    assert re.search(r"(?m)^\s+fast-build\s+\S+\s*$", status)
