...
ZAIGR_BIN = os.path.abspath(
    os.environ.get("ZAIGR_TEST_BINARY") or os.path.join(REPO_ROOT, "zaigr")
)
...
def zaigr_bin():
    """Return the explicitly selected executable; never build inside a worker."""
    if not os.environ.get("ZAIGR_TEST_BINARY"):
        pytest.fail(
            "No test binary selected. Use make test-cli/test-builtin, or run "
            "make build then ZAIGR_TEST_BINARY=$PWD/zaigr pytest ..."
        )
    if not os.path.isfile(ZAIGR_BIN) or not os.access(ZAIGR_BIN, os.X_OK):
        pytest.fail(f"Selected test binary is missing or not executable: {ZAIGR_BIN}")
    return ZAIGR_BIN
...
def project_factory(test_root, zaigr_bin, zaigr_home):
    ...
    def factory(name=None):
        """Create and track a project for fixture-owned VM teardown."""
        nonlocal created
        created += 1
    ...
    for project in reversed(created_projects):
        try:
            ...
            stop_errors.append(f"{project.cwd}: {exc}")
            continue
        text = stdout + stderr
        if rc != 0 and "project VM is not running" not in text and "requires a project" not in text:
            stop_errors.append(f"{project.cwd}: {err_msg(stdout, stderr)}")
    assert not stop_errors, "Project VM teardown failed:\n" + "\n\n".join(stop_errors)
