...
func (store projectStore) writeSetupCaptureMetadata(meta setupCaptureMetadata) error {
	...
	if err := os.WriteFile(store.setupCaptureMetadataPath(), append(body, '\n'), 0644); err != nil {
...
func (store projectStore) clearSetupCaptureMetadata() error {
	if err := os.Remove(store.setupCaptureMetadataPath()); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove setup capture metadata: %w", err)
	}
	return nil
}

func (store projectStore) clearSetupCaptureObservedEndpoints() error {
	if err := os.Remove(store.setupCaptureObservedEndpointsPath()); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove setup capture observed endpoints: %w", err)
	}
	return nil
}

func (store projectStore) clearSetupCaptureCommands() error {
	if err := os.Remove(store.setupCaptureCommandsPath()); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove setup capture commands: %w", err)
	}
	if err := os.Remove(store.setupCaptureLegacyCommandsPath()); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove legacy setup capture commands: %w", err)
	}
	return nil
}
