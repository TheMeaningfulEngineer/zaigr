...
const setupCaptureDomainLogReadyMarker = "__ZAIGR_SETUP_CAPTURE_DOMAIN_LOG_READY__"
...
func cmdProjectSetupCaptureStart() error {
	lock, err := lockCurrentProjectCapture("project setup capture start")
	if err != nil {
		return err
	}
	defer lock.releaseWithWarning()
	if err := lock.recover(currentProjectStore(), "project setup capture start"); err != nil {
		return err
	}
	...
	if meta, has, err := store.readSetupCaptureMetadata(); err != nil {
		...
		return activeSetupCaptureRefusalError("zaigr project setup capture start", meta)
	}
...
func startNewSetupCapture(store projectStore, confirmRunningVM bool) error {
	...
	for _, session := range sessions {
		if session.User == "root" {
			return fmt.Errorf("cannot start project setup capture: a root shell is running in another terminal")
		}
	}
...
func requireNoActiveSetupCapture(command string) error {
	return requireNoActiveSetupCaptureForStore(currentProjectStore(), command)
}

func requireNoActiveSetupCaptureForStore(store projectStore, command string) error {
...
func cmdProjectSetupCaptureReview() error {
	lock, err := lockCurrentProjectAllowCapture("project setup capture review")
	if err != nil {
		return err
	}
	defer lock.releaseWithWarning()
...
func cmdProjectSetupCaptureAccept() error {
	lock, err := lockCurrentProjectAllowCapture("project setup capture accept")
	if err != nil {
		return err
	}
	defer lock.releaseWithWarning()
	...
	if hasOverlay && running != nil && running.ImagePath != "" && running.ImagePath != meta.OverlayImagePath {
	...
	} else if hasOverlay && running != nil && running.ImagePath == meta.OverlayImagePath {
		rootKeyPath := filepath.Join(zaigDir(), "root-ssh.key")
		if err := removeAllTemporaryCaptureAllowAllFirewallRules(running.Port, rootKeyPath); err != nil {
			return fmt.Errorf("remove temporary capture firewall before accepting: %w", err)
		}
		if _, err := stopProjectVMInStore(store, false); err != nil {
			return fmt.Errorf("stop capture VM before accepting: %w", err)
		}
	}
	meta.ObservedFirewall = observed
	...
	if hasOverlay {
		...
		definition, err := loadGlobalSetupDefinition(meta.Name)
		if err != nil {
			return err
		}
		if err := acceptRunningSetupCapture(store, definition); err != nil {
			return err
		}
	}
...
func acceptRunningSetupCapture(store projectStore, definition setupDefinition) error {
	...
	metadataPaths := []string{
		...
		store.firewallPath(),
		store.setupCaptureMetadataPath(),
		store.setupCaptureObservedEndpointsPath(),
		store.setupCaptureCommandsPath(),
		store.setupCaptureLegacyCommandsPath(),
	}
	return updateProjectMetadataTransaction(store, metadataPaths, func() error {
		if err := store.writeAppliedSetups(upsertAppliedSetup(records, appliedSetup{Name: definition.Name, Version: definition.Version, ProjectLocal: definition.ProjectLocal})); err != nil {
			return err
		}
		...
		return store.clearSetupCaptureCommands()
	})
}
...
func acceptSetupCaptureOverlay(meta setupCaptureMetadata, script string) (string, error) {
	_ = script
	store := currentProjectStore()
	newImagePath := store.authoritativeImagePath()
	candidate := newImagePath + ".capture-accept-wip"
	_ = os.Remove(candidate)
	defer func() { _ = os.Remove(candidate) }()
	if err := compactToDirectProjectOverlay(meta.OverlayImagePath, candidate); err != nil {
		return "", err
	}
	definition, err := loadGlobalSetupDefinition(meta.Name)
	if err != nil {
	...
	if err := os.Remove(meta.OverlayImagePath); err != nil && !os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, ":: Warning: accepted capture overlay cleanup will be retried: %v\n", err)
	}
	return newImagePath, nil
}

func cmdProjectSetupCaptureDiscard() error {
	lock, err := lockCurrentProjectAllowCapture("project setup capture discard")
	if err != nil {
		return err
	}
	defer lock.releaseWithWarning()
...
func discardSetupCaptureState(store projectStore, meta setupCaptureMetadata, prompt bool) (bool, error) {
	...
	if meta.OverlayImagePath != "" && running != nil && running.ImagePath == meta.OverlayImagePath {
		...
		rootKeyPath := filepath.Join(zaigDir(), "root-ssh.key")
		if err := removeAllTemporaryCaptureAllowAllFirewallRules(running.Port, rootKeyPath); err != nil {
			return false, fmt.Errorf("remove temporary capture firewall before discard: %w", err)
		}
...
func startSetupCaptureDomainLog(ctx context.Context, port string, rootKeyPath string, outputPath string) (*exec.Cmd, func() error, error) {
	...
	args = append(
		...
		"-p", port,
		"root@localhost",
		"env", "ZAIGR_INSIDE_DOMAIN_LOG_READY_MARKER="+setupCaptureDomainLogReadyMarker,
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if line == setupCaptureDomainLogReadyMarker {
				select {
				case ready <- struct{}{}:
				default:
				}
	...
	case <-done:
		_ = output.Close()
		return nil, nil, fmt.Errorf("capture endpoint log stopped before becoming ready")
	case <-time.After(10 * time.Second):
		_ = cmd.Process.Kill()
		<-done
		_ = output.Close()
		return nil, nil, fmt.Errorf("capture endpoint log did not become ready")
	}
...
func setupCaptureBaseImage(store projectStore) (string, error) {
	...
	return "", fmt.Errorf("project image missing: %s; run 'zaigr project clean' to recreate it", filepath.Base(imagePath))
}
...
func buildCaptureSetupScript(name string, observed []string, commands []setupCaptureCommand, startedAt string) string {
	...
	_, _ = fmt.Fprintf(&script, "# Captured setup: %s\n", name)
	if startedAt != "" {
		_, _ = fmt.Fprintf(&script, "# Captured: %s\n", startedAt)
	}
	if len(observed) > 0 {
	...
	for _, command := range commands {
		...
		if command.HasExitStatus {
			_, _ = fmt.Fprintf(&script, "# Returned %d\n", command.ExitStatus)
		}
